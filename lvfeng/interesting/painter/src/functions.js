...
function open_from_FileList(files, user_input_method_verb_past_tense){
	...
	if(file){
		open_from_File(file, function(err){
			if(err){ return show_error_message("打开文件失败:", err); }
		});
	}
}
...
function are_you_sure(action, canceled){
	if(saved){
		...
		var $w = new $FormWindow().addClass("dialogue-window");
		$w.title("画图");
		$w.$main.text("保存文件 "+file_name+"?");
		$w.$Button("保存", function(){
			$w.close();
			file_save();
			action();
		}).focus();
		$w.$Button("放弃", function(){
			$w.close();
			action();
		});
		$w.$Button("取消", function(){
			$w.close();
			canceled && canceled();
		});
